...
대신 복사 생성자/ 복사 대입 연산자의 코드 본문이 비슷하게 나온다면, 양쪽에서 겹치는 부분을 별도의 멤버 함수에 분리 해놓은 후에 이 함수를 호출하도록 만드는 방법이 있다, 주로 private로 두고, 이름이 init~로 시작한다. 안정하고 검증된 방법이니 코드를 중복을 처리할 때 괜찮은 방법이다.
