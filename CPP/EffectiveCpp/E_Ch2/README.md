...
사용자가 에러가 발생할만한 가능성이 있는 코드를 원하는 타이밍에 호출할 수 있으므로, 에러에 잘 대처 할 수 있음 <br>
  ...
  소멸자에서는 예외가 빠져나가면 안됨, 만약 소멸자 안에서 호출된 함수가 예외를 던질 가능성이 있다면, 어떤 예전 것에 대한 기본 클래스
  
--------------------------------------------------------

##  2-9 객체 생성 및 소멸 과정 중에는 절대로 가상 함수를 호출하지 말자

객체의 생성 및 소멸 과정에서 가상 함수를 호출하면 **미정의 동작**이 발생할 수 있기 때문에, 호출하면 안된다. <br>
밑의 코드를 보자 
...
## 2-10 대입 연산자는 *this 의 참조자를 반환 하게 하자
...
```cpp
a[i] = a[j]; //자기 대입에 대한 가능성을 아름드리 품은 문장
*px = *py; //자기 대입의 가능성을 하나 가득 품은 문
```
...
이렇게 순서를 바꾸면 new Bitmap 부분에서 예외가 발생해도 delete가 실행되지 않아 pb는 변경되지 않고, 자기 참조가 일어나더라도 안전한 동작이 발생함 <br>
효율에 너무 신경 쓴 나머지, 일치성 테스트를 함수 앞에 붙여 놓을 수 도 있지만, 근데 생각해을 해보자, 프로그램에서 자기 대입이 얼마나 자주 일어날까? 그리고 일치성 테스트는 공짜가 아니다 일치성 검사 코드가 들어가면, 그만큼 코드(소스 코드와 목적 코드 둘다)가 커지는데다가, 처리 흐름에 분기를 만들게 되어 실행 시간 속력이 줄어 들 수 있어 CPU의 명령어 선행 인출(instruction prefetch), 캐시, 파이프라이닝 등의 효과가 떨어질 수 있음 <br> 

그럼 복사 후 맞 바꾸기 (copy and swap)으로 해결 해보자
...
Widget& Widget::operator= (const Widget& rhs){
  ...
  Widget temp(rhs); //rhs에 대한 데이터 사본을 하나 만든다
  swap(temp); //*this의 데이터를 그 사본의 것과 맞바꾼다.
  return *this;
}
```

이 방법은 C++의 2가지 특징을 이용해서 조금 다르게 구현도 가능하다.  <br>
1. 클래스의 복사 대입 연산자는 argument를 값으로 취하도록 선언하는 것이 가능하다
2. 값에 의한 전달(call by value)을 수행하면 전달된 대상의 사본이 생긴다.
...
하지만 이코드는 명확성을 제물로 바치는 격이지만, 객체를 복사하는 코드가 함수 본문으로부터 매개변수의 생성자로 옮겨졌기 때문에, 컴파일러가 더 효율적인 코드를 생성할 수 있는 여지가 만들어 진다.

  - **2-11 정리**

    operator=을 구현할 때, 어떤 객체가 그 자신에 대입되는 경우를 제대로 처리하도록 만들자. 원본 객체와 복사대상 객체의 주소를 비교해도 되고, 문장의 순서를 적절히 조정할 수 있으며, 복사 후 맞 바꾸기 기법을 써도 된다. <br>
    두 개 이상의 객체에 대해서 동작하는 함수가 있다면, 이 함수에 넘겨지는 개체들이 사실 같은 객체 인 경우에 정확하게 동작하는지를 확인하자.
    
-----------------------------------------------------------------

## 2-12 객체의 모든 부분을 빠짐없이 복사하자

객체의 안쪽 부분을 캡슐화된 객체 지향 시스템 중 설계가 잘 된 것들을 보면, 객체를 복사하는 함수가 딱 2가지만 있는걸 알 수 있다. **복사 생성자**와 **복사 대입 연산자** 이 둘을 통틀어 **객체 복사 함수(Copying Function)** 이라고 한다. <br>
컴파일러가 생성한 복사 함수는 객체의 모든 부분을 빠짐 없이 복사해주지만, 프로그래머가 직접 복사 함수를 선언한다면, 구현한 복사 함수가 틀린 경우에도 알려주지 않는다.

다음 코드를 보자, 이 코드는 customer을 나타내는 클래스가 하나 있고, 복사 함수는 프로그래머가 직접 구현하여, 복사 함수를 호출할 때마다 로그를 남기도록 구현되었다. 

```cpp
void logCall(const string& funcName); //로그 기록을 만든다.
...
Customer& Customer::operator= (const Customer& rhs){
  ...
  logCall("Customer copy Assignmnet operator");
  name = rhs.name; //rhs의 데이터 복사 
  return *this; 
}
```

위의 코드를 보면, 문제가 없어 보여도, 데이터 멤버 하나를 Customer에 추가하면 문제가 생길 수 있다. 
...
이렇게 데이터 멤버를 추가하면 복사 함수의 완전 복사가 아닌 **부분 복사(partial copy)**가 된다. <br>
하지만 컴파일러는 이를 프로그래머에게 고지해주지 않는다. <br>
즉, 데이터 멤버를 추가 했으면, 추가한 데이터 멤버를 처리할 수 있도록 복사 함수를 다시 작성해야 한다. (생성자 및 operator= 함수 갱신)

또 다른 코드를 보자 <br>
이 코드는 클래스 상속에서의 복사이다. 

```cpp
class PriorityCustomer : public Customer{ //Customer의 자식 클래스인 PriorityCustomer 클래스 
...
PriorityCustomer클래스의 복사 함수는 PriorityCustomer의 모든 것을 복사하고 있는 것처럼 보이지만, Customer로부터 상속한 데이터 멤버들의 사본은 복사가 되지 않고 있음을 알 수 있다 <br> 
또한, PriorityCustomer의 복사 생성자에는 기본 클래스 생성자에 넘길 argument가 명시되지 않아 있어서, Customer의 생성자는 기본 생성자에 의해 초기화됨. <br>
그리고 PriorityCustomer의 복사 대입 연산자의 경우에는 기본 클래스의 데이터 멤버를 건드릴 시도도 하지 않기 떄문에, 기본 클래스의 데이터 멤버는 변경되지 않고, 그대로 있게 된다. <br>

그러므로, 복사 함수를 직접 구현한다면, 기본 클래스 부분을 빠트리지 않고 복사하자.

```cpp
// customer(rhs)는 본 클래스의 복사 생성자를 호출한다 
...
객체의 복사 함수를 작성할 때 다음 2가지를 꼭 확인하자 <br> 
1. 해당 클래스의 데이터 멤버를 모두 복사하고,
2. 이 클래스가 상속한 기본 클래스의 복사 함수도 꼬박꼬박 호출 해주도록 하자.

복사 생성자와 복사 대입 연산자가 비슷해서, 한쪽에서 다른쪽을 호출하게 만들어서 코드 중복을 피할려고 하는 경우가 있는데, 이런 식으로 작성하면 여러 문제가 발생한다. <br>
복사 대입 연산자에서 복사 생성자를 호출하는 것이 말이 안 되는 발생인데, 이미 만들어져 버젓이 존재하는 객체를 **생성**하고 있기 때문이다.  또한, 특정한 조건에서 객체의 데이터가 훼손될 수 있기 때문이다. <br>
반대로 복사 생성자에서 복사 대입 생성자를 호출하는 것은 어떨까? 이것 또한 넌센스 퀴즈급이다. <br>
생성자의 역할은 새로 만들어진 객체를 초기화하는 것잊만, 대입 연산자의 역할은 **이미** 초기화가 끝난 객체에게 값을 주는 것이다. 초기화된 객체에만 적용된다는 뜻이고, 만약에 생성 중인 객체에다가 대입이란 것은 초기화된 객체에 대해서만 의미를 갖는 동작을 '아직 초기화도 안 된' 객체에 대해서 한다는 것이다. **그냥 말이 안되니 하지말자**

대신 복사 생성자/ 복사 대입 연산자의 코드 본문이 비슷하게 나온다면, 양쪽에서 겹치는 부분을 별도의 멤버 함수에 분리 해놓은 후에 이 함수를 호출하도록 만드는 방법이 있다, 주로 private로 두고, 이름이 init~로 시작한다. 안전하고 검증된 방법이니 코드를 중복을 처리할 때 괜찮은 방법이다.

  - **2-12 정리**
    
    객체 복사 함수는 주어진 객체의 모든 데이터 멤버 및 모든 기본 클래스 부분을 빠뜨리지 말고 복사해야 한다. <br>
    클래스의 복사 함수 2개를 구현할 때, 한쪽을 이용해서 다른 쪽을 구현하는 시도는 절대 하지말자, 그 대신 공통된 동작을 하는 것은 제 3의 함수에 분리해 놓고 양쪽에서 이것을 호출하도록 만들자.
