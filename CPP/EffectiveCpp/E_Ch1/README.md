...
효과적으로 활용하기 위해선 C++는 C++스타일답게 사용하자. 
  ...
  - [1-4 객체를 사용하기 전에 반드시 그 객체를 초기화하자](#1-4-객체를-사용하기전에는-반드시-객체를-초기화하자)
...
현대의 C++는 **다중패러다임 프로그래밍 언어(Multiparadigm Programming Language)** 이며, 절차적인 프로그래밍을 베이스하여 **객체지향, 함수, 일반화 프로그래밍(템플릿)을 포함한 메타프로그래밍 개념까지 지원하는 언어**로 자리 잡음. <br>
...
이 개념에 2개의 매우 거대한 개념이 자리 잡고 있는데 1. 비트수준 상수성(bitwise constness) or 물리적 상수성(physical constness) 2. 논리적 상수성(logical constness)임. 
  ...
  1. 비트수준 상수성(bitwise constness) or 물리적 상수성(physical constness) <br>
  ...
  2. 논리적 상수성(logical constness) <br>
...
위의 코드에서 operator[]는 동일한 동작을 하지만 const 유무로인해 중복된 코드를 작성하게 됨. <br>
이는 컴파일 시간, 유지보수, 코드 크기 증가 등을 부르기 때문에 다음 코드처럼 핵심 기능을 하나 구현하고, 이를 호출하는 식으로 만들다면 더 좋은 방식이 될 수 있음 <br>
...
public:
  const char& operator[](size_t position) const { return text[position]; } //이전과 동일 

  // 상수버전인 op[] 호출하고, op[]의 반환 타입에 캐스팅을 적용하여 const를 때어냄.
  // 그리고 *this의 타입에 const를 붙여 op[]의 상수버전을 호출
...
```
  이와 반대로 비상수 멤버 함수를 호출하면 안되냐는 생각을 할 수 잇는데, 상수 멤버 함수는 해당 객체의 논리적인 상태를 바꾸지 않겠다고 약속했기 때문에 상수 함수에서 비상수 함수를 호출하면 안됨 
