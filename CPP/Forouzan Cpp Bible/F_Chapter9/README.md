 ...
 * #### 포인터 자료형과 포인터 변수
   + ##### 포인터 자료형
     ...
     Circle*
     ```
     위의 코드의 예시가 복잡 자료형, **어떤 자료형의 포인터를 만들 때는 자료형 뒤에 (*)를 붙임**
 ...
 int num;
 double *p = &num; //int 자로형 변수의 주소 할당 불가 -> 컴ㅂ파일 오류 
     ```
 ...
 + ##### 간접참조
   ...
   이때 *기호를 사용하는 간접 참조 연산자를(indirection operation)를 사용 <br>

   ```cpp
     *pFlag; //pFalge의 주소에 저장된 값
     *pScore; //pScore의 주소에 저장된 값
     *pAverage; //pAverage의 주소에 저장된 값
   ```
+ ##### 연산자 정리
