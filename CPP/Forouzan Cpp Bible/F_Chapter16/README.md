...
-----------------------------------------------
  ...
  * ### 16-1. 개요
    ...
    데이터 소스와 싱크는 프로그램과 연결되어 있지 않아, 데이터 흐름을 제어하고 읽고 쓰려면 프로그램과 이들을 연결할 중간 다리가 있어야하는데 이를 **스트림(stream)** 이라고 부름. 스트림은 입력 스트림과 출력 스트림이 있고 입력 스트림은 데이터 소스와 프로그램 사이의 중간 다리를, 출력 스트림은 프로그램과 데이터 싱크 사이의 중간 다리 역할을 수행함 <br>
...
-------------------------------------------------
  ...
  * ### 16-2. 콘솔 스트림
    ...
    - 콘솔 객체
      ...
      그래서 시스템이  **cin** 이라는 이름으로 인스턴스화 하고 키보드와 연결하고나서 제공, 참고로 cin은 프로그램이 종료될 때 소멸되고 키보드와 자동으로 연결이 해제됨.
    ...
    - 상태 데이터 멤버
      ...
      또한 상태와 관련된 멤버 함수가 있는데 operator void*()는 객체를 자료형으로 변환하는 자료형 변환 연산자 오버로드 함수임, operator!()는 논리 부정 연산자 오버로드 함수로 operator void*()에서 bool type로 변환되는 것을 리턴하는 함수라고 생각하면 됨 <br>
...
------------------------------------------------------------------------------------------
  ...
  * ### 16-3. 파일 스트림
    ...
    ifstream, ofstream, fstream이 있는데 이 클래스들은 istream, ostream, iostream으로부터 상속되어 있기에, 부모 클래스에 있던 모든 멤버 함수를 상속 받고, 인스턴스화와 파일 열기위한 몇가지 기능을 가짐
    ...
    - 이진 입출력
      ...
      위의 함수들은 메모리에 저장된 바이트와 스트림 버퍼 사이에 자료형 변환 없이 모두 문자로 처리함 그렇기 때문에 문자 배열로 변환하거나 반대로 변환하려면 자료형을 지정할 수 있는 장치가 필요함 <br>
    ...
    - 이진 파일의 임의 접근
      ...
      기본 자료형의 데이터 멤버만을 가지는 객체의 경우, 모든 데이터멤버 크기의 합으로 저장되는 크기를 구할 수 있음 <br>
