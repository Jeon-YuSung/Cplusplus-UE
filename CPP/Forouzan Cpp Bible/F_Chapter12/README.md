...
* 가상 함수
  ...
  * 생성자와 소멸자
    ...
    * 1. 가상 생성자 불가 : 생성자도 멤버 함수이지만, 생성자의 이름은 부모 클래스와 자식 클래스가 무조건 달라, 가상 클래스 함수가 될 수 없음 <br>
    * 2. 가상 소멸자 : 소멸자의 이름도 부모 클래스와 자식 클래스가 무조건 다름, 하지만 소멸자는 이름으로 호출하지 않아, 가상 소멸자라는 것이 존재. <br>
...
가상 베이스를 사용하면, 자식 클래스에 부모 클래스의 멤버가 직접적으로 들어가지 않음. <br>
