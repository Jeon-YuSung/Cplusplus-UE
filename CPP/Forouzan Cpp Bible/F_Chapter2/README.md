...
## using namespace std;
-------------------------
 std::를 입력하지 않은 경우 알아서 std::를 넣어서 판단하라고 컴파일러에 전달하는 것, 
...
Boolean type은 참(true) or 거짓(false)를 나타내는 값 
...
소수점을 갖는 숫자를 부동 소수점(floating-point)라고 함 

float, double, long double 3가지 타입이 부동 소수점을 정의하고, 부동 소수점은 모두 부호가 있는 숫자이다.

IEEE표준에 따라서 메모리 영역을 차지.

보통, float <= double <= long double 이라는 상대적 크기를 가짐 
