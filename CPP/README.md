 ...
 * A-1. [Forouzan C++ Bible - Behrpiz A.Forozan, Richard F.Gillberg 저, 윤인성 옮김](https://product.kyobobook.co.kr/detail/S000001743779) - 입문용으로 적합, C언어를 몰라도 큰 문제가 없는 책으로 프로그래밍 처음이라면 추천하고 혹은 명품 C++ 프로그래밍이나 윤성우의 열혈 C++ 프로그래밍, 모두의 코드(A-5)를 봐도 좋다. 
 * A-2. [A Tour of C++ Third Edition - 비야네 스트롭스트룹](https://product.kyobobook.co.kr/detail/S000213611373) - 초보~중급, 프로그래밍 경험이 있는 사람이라면 읽어봐도 좋다, C++ 창시자가 쓴 표준 C++에 대한 개요를 설명하기 때문에 꼭 C++이 아니여도 다른 언어 경험있는 사람들에게 추천하는 책이다.  
 ...
 * A-4. [Effective C++ - 스콧 마이어스](https://product.kyobobook.co.kr/detail/S000001962302) - 유명하다, 이펙티브 모던 C++까지 한 셋트이다. C++26까지 나온 시점에서 살짝 문법이 달라지거나 없어진 문법이 있을 수 도 있다. 아마 이펙티브가 C++03 기준이고 C++11도 반영되었다고 하는데 구식이라고 한다. **Effective Modern C++**(번역판은 번역이 깔끔하지 않는다는 평이 있어서, **무조건 원서 봐라**)이 C++11/14 반영으로 알고 있다. 오래된 책이지만, 추천하는 이유는 50가지의 지침를 제공해주고, 엄청 좋은 책이라 C++ 개발자라면 **무조건** 봐야하는 책이기 때문이다. 난이도는 중~고급 
 * A-5. [모두의 코드 C++ (씹어먹는 C++)](https://modoocode.com/) - 입문용 추천하는 책이며, 서점가기 귀찮고, 인터넷으로 공부하고 싶으면 추천 쉬운 예제와 풍부한 설명이 있다, C++는 C언어를 어느정도 알고 있다고 가정하에 집필했기 때문에, 모두의 코드 C언어를 보고, 그 다음부터 C++를 보는걸 추천한다. 
 ...
 * A-11. [MS Learn Cpp Docs](https://learn.microsoft.com/ko-kr/cpp/cpp/?view=msvc-170) - 마이크로소프트의 CPP 공식문서 궁금한 기능이나 함수가 있으면 여길 참고하면 된다.
