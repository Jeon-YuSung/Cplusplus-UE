...
## 5-7 논리 오류
컴파일러와 링커가 찾아낸 오류를 우선 제거하면 프로그램은 실행된다. 그러나 이렇게 프로그램을 처음 실행하면 아무런 결과가 나오지 않거나 프로그램이 생성한 출력이 잘못된 경우가 대부분이다. 
이는 프로그램을 구성하는 논리에 대한 결함이 있거나, 생각한대로 코드를 작성하지 않거나.  if문에서 바보 같은 오류가 있다던가... 이러한 여러가지 이유로 컴파일러는 문법 체크때 OK해주지만, 생각한대로 결과가 나오지 않는 것을 **논리 오류(Logic Error)** 라고 한다. 이는 프로그래머가 직접 찾아서 오류를 고쳐야한다. 
(팁을 주자면 Visual Studio에서 프로그램을 실행할 때 단축기 f9~f12키를 잘 사용해보자) 

----------------------------------------------------------------------------------------

## 5-8 평가
항상 프로그래머가 원하는 결과가 나오지는 않는다. 예를들어 도형의 넓이를 구하는 프로그램을 만들다고 생각 했을때 -34.21이 출력되면 잘못된 값을 출력했다는 것을 알 수 있다. 도형의 넓이가 음수일리가 없기 때문에, 그럼 만약에 213.4128039라는 값이 나오면 또 이건 옳은 결과일까? 이건 도형의 공식을 확인해봐야 알 수 있다. <br>
이런 바보 같은 결과를 내는 프로그램을 배포하기 전에 답이 옳은지 확인을 해야한다. <br> 
즉, 대략적이라도 정답이 무엇인지를 생각해두지 않으면, 프로그램의 결과가 옳은지 판단할 근거가 없다는 점이니까 항상 다음과 같은 질문을 생각해야한다. 

1. 특정 문제에 대한 해답이 적절한가?
2. 적절한 결과임을 어떻게 알 수 있는가?

완벽한 답이나 올바른 답을 요구하는 게 아닌, 결과가 터무니없지 않음을 확인하고자 하는거다. 결과가 그럴 듯하면 다음 단계로 진행할 수 있기 때문이다. <br>
 평가(Estimation)는 상식과 몇 가지 요인에 대한 간단한 산술을 조합하는 고도의 기술이기에, 미릿속으로 평가하는 것 보다는 어딘가에 글이나 그림을 그리는걸 추천한다. <br>
 그리고, 평가라는 범주에 포함되는 비공식적인 기술들을 일컬어 추정(Guesstimation)이라고 하고, 평가가 약간의 추측과 약간의 계산을 조합한 개념이기 때문이다. <br>
 그러니, 정확한 답이 아닌 충분한 추정만 해도 된다. 

----------------------------------------------------------------------------------------

## 5-9 디버깅
 오류는 항상 발생할 수 있다.... 아무리 작은 프로그램이라도, 하지만 보통은 단순하고 작은 프로그램은 올바르게 실행 될 것이다. 만약에 거대한 프로그램에서 오류가 나지 않고, 첫 시도에 에러없이 정상적으로 작동을 한다면 대단한 것이다. 이는 희귀한 케이스다. <br>
아무튼 코드를 작성한 후에는 오류를 찾아 없애야한다. 이러한 과정을 **디버깅(Debugging)** 이라과며, 이때의 오류를 **버그(BUG)** 라고 한다. <br>
버그는 진공관 시절.... 포트란, 코볼 시절... 누가 발명했든, 어느 시절이든 버그는 항상 존재하였다. 그리고 세밀하게 오류를 찾아 제거하는 일을 디버깅이라고 한다. 
 디버깅 과정을 알아보자. 
 ...
 기본적으로 이 과정을 **계속** 반복한다. (1~2번이 아님... 수백, 수천 혹은 몇 년이 걸릴수도 있음)
이 과정에서 뭔가 작동하지 않으면 문제의 원인을 찾아 고쳐야 한다. 
디버깅이 과정이 가장 지루하고, 시간 낭비가 되는 일이기에, 이러한 과정의 시간을 최소화하고 싶다면, 설계와 프로그래밍 과정에서 충분한 고려를 해야한다.         

**프로그램이 정말 올바르게 동작하는지 어떻게 알 수 있을까?**

이 질문이 디버깅 측면에서 핵심적인 질문이다. 이 질문에 대답을 할 수 없으면 길고 지루한 디버깅 회의를 해야할 수 있으며 사용자(유저)는 당황스럽거나 황당한 일을 겪을 수 있다. <br> 
이 질문에 대답 하는 데 도움이 되는 무엇이든 디버깅을 최소화하고 정확하게 관리하기 쉬운 프로그램을 만드는데 도움의 된다. 기본적으로는 버그가 숨을 곳이 없게 프로그램을 설계해야 한다. 이는 프로그램을 구조화해서 오류의 가능성을 줄이고 소리 없이 생겨나는 오류를 쉽게 찾을 수 있도록 노력하면 된다. 

**실용적인 디버깅 조언**

코드의 첫 행을 작성하기 전에 디버깅을 고려하고, 만약에 에러가 발생할 경우 오류를 어떻게 보고할지 결정하자. 그리고 다음과 같이 프로그램을 문서화 or 명세화해서 읽기 쉽게 만들어서 버그를 발견할 가능성을 높히자.
1. 주석 → 단순히 주석을 많이 달아라가 아니라 코드에서 명확히 드러나는 사실을 설명하기 보다는 **잘 드러나지 않는 사실을 최대한 명확하고 간결하게** 주석달자.
2. 프로그램의 이름 및 목적
3. 누가, 언제 코드를 작성 했는지 (협업)
4. 버전 번호
5. 복잡한 코드를 설계한 이유(의도)
6. 전반적인 설계 방식
7. 소스코드를 조직화한 방법
8. 입력에 대한 가정
9. 코드에 빠진 부분 및 처리하지 못 한 경우
10. 의미가 잘 드러나는 이름을 사용하자 → 긴 이름이나 의미 없는 이름을 사용하는 것이 아닌 변수명을 정할 때 의미가 있는 걸로 사용하자 ex) int a; -> int count;
11. IDE의 도움을 받을 수 있으나, IDE가 모든 일을 해주지 않으며, 결국 책임은 프로그래머에게 있다.
12. 논리적으로 단일한 작업을 수행하는 함수 단위로 코드를 잘게 나누자 → 한 두 페이지 이상의 함수를 피하자. 대부분의 함수는 이보다 훨씬 짧다.
13. 코드를 복잡하게 나열하지 말자. → 중첩된 루프와 중첩된 if구문, 복잡한 조건 등을 피하자 물론 필요한 경우가 있지만, 복잡한 코드일 수록 버그가 숨기 좋은 공간이다.
14. 되도록 직접 코드를 작성하지 말고 라이브러리 기능을 활용하자.
추상적이지만 프로그래밍에 익숙해지면 이런 조언은 도움이 된다. <br>
그리고 컴파일러는 가장 큰 도움이 된다 특히, 컴파일러의 경고 메시지를 잘 살펴보자

프로그램을 컴파일하고 링크한 후에 가장 어려운 과정을 거치게된다. 바로 프로그램이 의도대로 동작하지 않는 이유를 찾아내는 과정이다. 
출력을 살펴보고 프로그램이 어떻게 그런 결과를 내는지 밝혀낸다. <br>
그리고 버그를 찾을 때는 옳다는 확신이 들 때 까지 모든 코드 구문을 하나씩 살펴본다. 

	- 문제점을 찾을 수 없다면, 실제로 작성한 코드가 아니라 프로그램 기대하는 바를 바라보고 있을 수도 있다. 
	- 혹은 마지막으로 올바른 출력을 생성한 지점과 그 다음 출력을 하는 부분(혹은 출력이 누락된 곳) 사이에 너무 많은 코드를 실행하고 있을 수 도 있다. 대부분의 프로그래밍 환경은 구문을 하나씩 실행하는 기능을 제공한다. 
	- 버그가 의심돼는 부분에서는 불변 조건(invariant - 항상 성립해야하는 조건)을 확인하는 구문을 추가해줄 수 있다.
	- 아무 효과가 없다면, 오류가 의심되지 않는 곳의 코드에도 불변 조건을 넣어보자, 그래도 문제를 찾을 수 없다면 엉뚱한 곳을 보고 있을 확률이 크다. 

불변 조건을 확인하는 구문을 assertion or assert라고 하고 #include <cassert>를 추가하고, assert(0);을 주로 사용한다. 

아무튼, 디버깅에는 답이 없지만 문제의 원인은 지저분한 코드에 버그가 숨어있을 확률이 크다는 거다. 그러니, 되도록 간단하고 논리적이며, 형식에 맞게 유지하면 디버깅 시간을 줄일 수 있다. 

----------------------------------------------------------------------------------------

## 5-10 사전 조건과 사후 조건

이제, 함수의 잘못된 인자를 처리하는 문제로 알아보자. <br>
기본적으로 함수를 호출하는 시점이 올바른 코드를 작성하고 오류를 잡아내기 가장 적합한 시점이다. 이는 함수가 논리적으로 분리된 계산 과정을 시작하는 부분이기 때문이다. 그리고 함수가 반환할 때 계산이 끝난다. 
```cpp
//argument는 a < b < c를 만족하는 양수이다.
...
```
위의 코드에서 주석으로 함수가 요구하는 인자에 대해 설명했고, if문으로 그 조건을 만족하는지 확인한다. 
이때 함수의 인자에 요구되는 조건을 **사전 조건(Pre-Condition)** 이고, 함수를 제대로 실행하려면 이 조건을 반드시 만족해야 한다. 
만약에, 사전 조건을 위반 했을 때 어찌해야 할까? 크게 2 가지 방법을 선택할 수 있다.
1. 무시한다 - 함수를 호출하는 모두가 올바른 인자를 넘겨준다고 가정하면서...
2. 확인한다. - 어떤식으로든 오류를 보고한다. 

```cpp
my_func(1,2,"ASdsad");
```
세 번째 인자가 정수가 아닌 문자열을고 사전조건이 위반되었다. 그러나 컴파일러는 확인할 수 없는 요구 사항/사전 조건을 주로 다룬다. 
그러니 사전 조건을 주석에항상 문서화를 하라.  주석이 없는 함수는 모든 가능한 인자의 값을 처리할 수 도 있다. <br>
그러나 함수를 호출하는 쪽에서 주석을 읽고 그대로 따를거라고 믿어도 될까? <br>
'호출되는 쪽에서 인자를 확인한다'는 규칙은 '함수가 사전 조건을 확인한다'라는 말로 해석이 가능하다. 
특별한 이유가 없으면 이 규칙을 따르고, 사전 조건을 확인하지 않는 가장 흔한 이유는 다음과 같다. 
1. 아무도 잘못된 인자를 줄 수 없는 경우
2. 사전 조건 확인이 코드의 속도를 느리게하는 경우
3. 검사하기가 너무 복잡한 경우

1번은 누가 호출할지 알고 있으면 합리적이고, 2번은 대부분 premature optimization의 예에 해당한다. 즉, 확인 과정이 정말로 부담이 된다는 사실을 확증하는 경우에만 확인 과정을 생략할 수 있다. <br>
마지막으로 3번은 사전 조건을 검사하는 과정이 함수를 실행하는 일보다 더 큰 작업인 경우 쉽게 찾을 수 있다. <br>

사전 조건을 작성하면 프로그램의 퀄리티를 높이는 데 도움이 된다. 즉, 함수가 요구하는 바를 강제적으로 생각하게 만들기 때문이다. 
만약에 사전 조건을 2 ~ 3줄로 명확하게 표현할 수 없으면, 무슨 일을 하고 있는지를 충분히 고민하지 않았다는 말로 더 고민해라. 
무엇보다 사전 조건을 확인하는 코드를 넣으면 많은 설계 실수를 방지 할 수 있다. <br>
디버깅하는 과정은 매우 귀찮기에 사전 조건을 명시하면 설계 오류는 물론이고, 함수 사용시의 오류를 빠르게 잡아내는 데에 도움이 된다. 

**사후 조건**

일반적으로 함수가 무엇을 반환하는지 명시해야 한다. 즉, 함수가 어떤 값을 반환한다는 말은 반환 값에 대한 약속을 한다는 말과 같다. 
```cpp
//사각형 넓이 계산
//인자가 잘못된 경우 Bad_area 예외 던짐
int area(int len, int wid){
	if(len <= 0 || wid <= 0) throw Bad_area();
	return len * wid;
}
```
위의 코드에서 사전 조건을 확인하지만 주석에는 명시하지 않았다, 그리고 계산결과가 옳았다고 가정해보자. 
```cpp
//사각형 넓이 계산
//사전 조건 : len, wid는 양수
// 사후 조건 : 넓이를 양수 반환.
...
int area(int len, int wid){
	if(len <= 0 || wid <= 0) error("area() 사전 조건");
	int Rectarea = len * wid;
	if(Rectarea <= 0) error("area() 사후 조건");
	return Rectarea;
}
```

사후 조건(Post-condition)은 완벽히 확인할 수 없지만, 적어도 결과가 양수라는 조건을 확인할 수 있다. 
사전 조건, 사후 조건은 코드의 기본적인 합리성을 제공해준다. 왜냐면 불변 조건, 정확성 그리고 테스팅과 밀정하게 관련되기 때문이다. 

----------------------------------------------------------------------

# 5-11 테스팅 

디버깅은 언제까지 해야할까? 모든 버그를 찾을 때 까지 계속 <br>
그럼 마지막 버그를 찾았다는건 어찌 알 수 있을까? 모른다. 알수 없음 <br>
완벽한 프로그램은 없다... 이걸 생각하자.. <br>
그리고 디버깅뿐만 아니라 오류를 찾는 데도 체계적인 방법이 필요한데 이를 **테스팅(Testing)** 이라고 한다. 

기본적으로 테스팅은 체계적으로 선택된 다수의 입력으로 프로그램을 실행한 후에 그 출력을 기대한 바와 비교하는 과정이다. <br>
이 때 주어진 한 가지 입력 집합으로 수행하는 테스트를 Test Case라고 하고, 현실의 프로그램은 수백만 개의 테스트 케이스를 요구할 수도 있다. 그렇기 때문에, 사람이 계속 입력을 타이핑하는 방법으로 수행할 수 없다. <br>
테스팅 할 때는 오류를 찾아내는 일이 좋은 일이고 중요한 일이기에 다음과 같은 자세로 임하자. 

1. 이 프로그램 혹은 코드를 망가트려서 버그를 반드시 내고 말 것이다.

이러한 자세로 실수를 개의치 않고 차분하게, 인내심을 갖고 체계적으로 가능한 프로그램 오류를 검증하자 <br>
