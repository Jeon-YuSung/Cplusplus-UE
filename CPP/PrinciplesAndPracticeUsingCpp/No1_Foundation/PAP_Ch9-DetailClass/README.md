...
### 복사 
 ...
 생성자는 클래스의 가장 중요한 멤버이기 때문문에, 생성자를 작성혀라면 객체를 생성할 때 무엇을 인자로 받고, 유효한 값이 무엇인지 혹은 불변 규칙이 무엇인지를 결정해야한다. 초기화에 대해 고민하는 일만으로도 오류를 방지하는데 큰 도움이 된다. 
...
### 기본 생성자 
 초기화하지 않은 변수는 심각한 오류의 원이니기에, 크랠스의 모든 객체가 초기화 되었다는 것을 보장하는 생성자가 필요하다. <br>
