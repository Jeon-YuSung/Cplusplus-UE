...
-----------------------------------------------------------
### 열거형(enum type)
Enum은 매우 간단한 사용자 정의 타입으로, 값(열거자)의 집합을 기호 상수로 정의하는 것이다. 
...
열거형의 몸체는 단순히 열거자enumerator의 목록이고, enum class에서 class의 의미는 열거거자가 해당 열거형의 유효 범위 안에 있다는 것이다. 즉, jan을 참조하려면 Month::jan으로 사용해야한다. 열br의
