...
여러번 고치고 개선을 했으나 슬슬 지저분해보이기 시작한다. 그러니 코드를 더 명확하고 짧게 만들 수 있는지, 주석이 제대로 되어 있는지를 확인해보자. <br>
프로그램은 타인에게 유지 보수를 목표로 인수인계 할 수 있는 정도가 아니라면, 그 프로그램은 완성된 프로그램이라고 할 수 없다. <br>

코드 수정하는 법은 책을 참고하고, 여기서는 주석 달기를 정리 해보겠다.

**주석 달기**
일단 주석이 뭘까?  프로그래밍에 있어 내용을 메모하는 목적으로 쓰이며, 소스 코드를 더 쉽게 이해할 수 있게 만드는 것이 주 목적이다.

자 그럼 주석을 쓸 때, 다음의 조건을 만족하는지 살펴보자.

1. 유효한가?(주석을 작성한 후에도 코드가 변경됐을 수 있다)
2. 독자(타인)에게 적합한가?(보통 그렇지 않다)
3. 코드를 이해하는데 방해가 될 만큼 너무 긴 주석인가?

3번의 견지에서 보면 코드에서 가장 잘 설명할 수 있는 부분은 코드 자체로 두는 편이 좋다. 그리고 누구나 코드만 봐도 알 수 있는 경우라면 주석을 안 넣어도 된다. 
주석은 코드로 설명하기 힘든 무언가를 위해 존재하고, 그 무언가는 '의도'이다. 코드는 그 수행을 내용을 명시하지만, 거기에 감춰진 의도를 보여주지 않기 때문이다. 
그러니 주석을 설명할 때 타인이 이해할 수 있고 짧고 명료하게 쓰는 것이 좋다. 
